...
def show_stats(dirnames, graphs):
    ...
    for dirname in dirnames:
        ...
        if row:
            ...
            for result in all_results:
                if result.get('tests_outcomes') and not result['tests_outcomes'][-1]:
                    failed_tests[dirname].append(result['testcase'])
    ...
    for dirname, tests in failed_tests.items():
        print(f"\n{dirname}:")
        for test in tests:
            print(f"  - {test}")

    return df  # If you want to return the DataFrame for further analysis
...
def run_test_real(
    ...
    print(f"Attempting to write results to: {results_fname}")
    
    try:
        results_fname.write_text(json.dumps(results, indent=4))
        print(f"Successfully wrote results to: {results_fname}")
        
        # Verify the file was written and can be read back
        if results_fname.exists():
            file_content = results_fname.read_text()
            print(f"File content length: {len(file_content)} characters")
            parsed_content = json.loads(file_content)
            print(f"Successfully parsed JSON content with {len(parsed_content)} keys")
        else:
            print(f"Warning: File {results_fname} does not exist after write attempt")
    except Exception as e:
        print(f"Error writing or reading results file: {e}")
        traceback.print_exc()
